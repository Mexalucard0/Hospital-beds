    ...
    Citizen.CreateThread(function ()
        ...
        if (health < 1000)  then
                exports.pNotify:SendNotification({
					text = "Du er ved at blive helbredt", 
					type = "success", 
					timeout = math.random(2995, 30000), 
					layout = "bottomRight", 
					queue = "right"
				})
            Citizen.Wait(5000)
            if InAction == true then
                while GetEntityHealth(PlayerPedId()) < 1000 and InAction == true do
                    Citizen.Wait(2000)
        ...
        elseif (health == 1000) then
			exports.pNotify:SendNotification({
				text = "Du har ikke behov for at blive helbredt", 
				type = "error", 
				timeout = math.random(2995, 30000), 
				layout = "bottomRight", 
				queue = "right"
